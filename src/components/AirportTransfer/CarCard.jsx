...
function CarCard({ transferData, onConfirm }) {
  const [currentStep, setCurrentStep] = useState(0); // Håller reda på nuvarande steg i bokningsprocessen
  const [showConfirmButton, setShowConfirmButton] = useState(false); // Hanterar visning av "Confirm"-knappen
  ...
  return (
    <div className="w-full max-w-[800px] bg-white rounded-lg p-6 shadow-md mx-auto mt-8">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex flex-col items-start">
          <h2 className="text-xl font-bold mb-4">
            Standard car – {transferData.returnTrip ? "Return" : "One way"}
          </h2>
          <img
            src={Car}
            alt="Car"
      ...
      {/* Detaljer för returresa om vald*/}
      ...
      {/* Confirmknapp efter guest details and payment form */}
      {showConfirmButton && currentStep >= 1 && (
        <div className="text-center mt-6">
          <Button
            size="large"
            buttonText="Confirm"
            onClick={onConfirm} // onConfirm funktion
          />
        </div>
      )}
    </div>
